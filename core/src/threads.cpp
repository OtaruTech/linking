...
#ifdef HAVE_LINKING_OS
# define __linking_unused
#else
# define __linking_unused __attribute__((__unused__))
#endif
...
typedef void* (*linking_pthread_entry)(void*);
...
struct thread_data_t {
    ...
    static int trampoline(const thread_data_t* t) {
        ...
        if (prio >= LINKING_PRIORITY_BACKGROUND) {
...
void linkingSetThreadName(const char* name) {
#if defined(__linux__)
...
int linkingCreateRawThreadEtc(linking_thread_func_t entryFunction,
                               void *userData,
                               const char* threadName __linking_unused,
                               int32_t threadPriority,
                               size_t threadStackSize,
                               linking_thread_id_t *threadId)
{
    pthread_attr_t attr;
    pthread_attr_init(&attr);
    pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);

#ifdef HAVE_LINKING_OS  /* valgrind is rejecting RT-priority create reqs */
    if (threadPriority != PRIORITY_DEFAULT || threadName != NULL) {
        // Now that the pthread_t has a method to find the associated
        // linking_thread_id_t (pid) from pthread_t, it would be possible to avoid
        ...
        t->userData = userData;
        entryFunction = (linking_thread_func_t)&thread_data_t::trampoline;
        userData = t;
    }
#endif
    ...
    int result = pthread_create(&thread, &attr,
                    (linking_pthread_entry)entryFunction, userData);
    pthread_attr_destroy(&attr);
    if (result != 0) {
        ALOGE("linkingCreateRawThreadEtc failed (entry=%p, res=%d, errno=%d)\n"
             "(linking threadPriority=%d)",
    ...
    if (threadId != NULL) {
        *threadId = (linking_thread_id_t)thread; // XXX: this is not portable
    }
    return 1;
}

#ifdef HAVE_LINKING_OS
static pthread_t linking_thread_id_t_to_pthread(linking_thread_id_t thread)
{
    return (pthread_t) thread;
}
#endif

linking_thread_id_t linkingGetThreadId()
{
    return (linking_thread_id_t)pthread_self();
}
 ...
 * Create and run a new thread.
 */
static bool doCreateThread(linking_thread_func_t fn, void* arg, linking_thread_id_t *id)
{
    HANDLE hThread;
    ...
    if (id != NULL) {
      	*id = (linking_thread_id_t)thrdaddr;
    }

    return true;
}

int linkingCreateRawThreadEtc(linking_thread_func_t fn,
                               void *userData,
...
linking_thread_id_t linkingGetThreadId()
{
    return (linking_thread_id_t)GetCurrentThreadId();
}
...
int linkingCreateThread(linking_thread_func_t fn, void* arg)
{
    return createThreadEtc(fn, arg);
}

int linkingCreateThreadGetID(linking_thread_func_t fn, void *arg, linking_thread_id_t *id)
{
    return createThreadEtc(fn, arg, "linking:unnamed_thread",
                           PRIORITY_DEFAULT, 0, id);
}

static linking_create_thread_fn gCreateThreadFn = linkingCreateRawThreadEtc;

int linkingCreateThreadEtc(linking_thread_func_t entryFunction,
                            void *userData,
                            const char* threadName,
                            int32_t threadPriority,
                            size_t threadStackSize,
...
void linkingSetCreateThreadFunc(linking_create_thread_fn func)
{
    gCreateThreadFn = func;
}

#ifdef HAVE_LINKING_OS
int linkingSetThreadPriority(pid_t tid, int pri)
{
    int rc = 0;
    ...
    if (pri >= LINKING_PRIORITY_BACKGROUND) {
        rc = set_sched_policy(tid, SP_BACKGROUND);
    } else if (getpriority(PRIO_PROCESS, tid) >= LINKING_PRIORITY_BACKGROUND) {
...
int linkingGetThreadPriority(pid_t tid) {
#if !defined(_WIN32)
    return getpriority(PRIO_PROCESS, tid);
#else
    return LINKING_PRIORITY_NORMAL;
#endif
}
...
status_t Thread::run(const char* name, int32_t priority, size_t stack)
{
    ...
    if (mCanCallJava) {
        ...
        res = linkingCreateRawThreadEtc(_threadLoop,
                this, name, priority, stack, &mThread);
    }
...
pid_t Thread::getTid() const
{
    ...
    // pid_t tid;
    // if (mRunning) {
    //     pthread_t pthread = linking_thread_id_t_to_pthread(mThread);
